...
└── images/            ← create this folder, put generated media here
    │
    │   # Photos (Section 2) — 11 files
    ├── hero-vr-student.webp
...
### 2.1 `hero-vr-student.webp` — MAIN LANDING HERO (photo)

Shows the core product: a student learning in VR.

> A young medical student wearing a modern white VR headset (Meta Quest 3 style), standing in a bright futuristic university atrium, hands extended interacting with invisible holographic content, soft blue daylight streaming through tall glass windows, cinematic editorial photography, shallow depth of field, white and pale sky blue palette, premium tech-education aesthetic, hopeful and reverent mood, face partially visible in profile, no text, no logos, 16:9 widescreen

**Midjourney:** `--ar 16:9 --style raw --stylize 250 --v 6 --no text logo`

---

> **Note on videos:** Every hero section on the site already supports video playback — the `<HeroBg>` component in `shared.jsx` auto-detects if a `./images/hero-*.mp4` file exists and plays it with the matching `.webp` as poster fallback. So as you generate each video and drop it in `images/`, the site starts using it automatically. If you skip a video, the photo stays. See **Section 3 — Video prompts** for all 7 video slots.
...
Only the main landing page (`/`) uses a looping background video. All other pages use photos from Section 2. The code is already wired up — just drop the MP4 into `images/hero-home.mp4` and the site starts playing it automatically. If you skip the video, the photo from Section 2.1 (`hero-vr-student.webp`) is used as fallback.

**Technical specs:**
- Length: 4–8 seconds
- Format: **MP4 (H.264)** — save to `images/hero-home.mp4`
...
- **Runway Image-to-Video** — upload `hero-vr-student.webp` from Section 2.1 and animate it (easiest path — keeps visual consistency with the photo)

### `hero-home.mp4` — MAIN LANDING HERO
...
The site falls back to `hero-vr-student.webp` (or the CSS gradient if that's missing too). Nothing breaks. Launch with just the photo if you want, add the video later.
...
|---------------|-----------------------------------|-------------------------------|
| `.img-horizon`| Hero, roadmap, HUMA cover         | `hero-vr-student.webp`        |
...
- [ ] Generate `hero-home.mp4` first (biggest impact)
